...
public class DimensionSpecialEffectsMixin implements IForgeDimensionSpecialEffects {
    ...
    /**
     * @author Apothicon
     * @reason Custom End lighting adjustments
     */
    @Override
    public void adjustLightmapColors(ClientLevel level, float partialTicks, float skyDarken, float blockLightRedFlicker, float skyLight, int pixelX, int pixelY, Vector3f colors) {
        if (AstrologicalJsonReader.customEndLighting && level.dimension().equals(Level.END)) {
            ...
            IForgeDimensionSpecialEffects.super.adjustLightmapColors(level, partialTicks, skyDarken, blockLightRedFlicker, skyLight, pixelX, pixelY, 
                colors.add(new Vector3f(
                    (-0.016f + (blockFactor / 1.9F)) - (blockFactor / 3), 
                    (-0.166f + (blockFactor / 1.2f)) - (blockFactor / 1.7f), 
                    -(blockFactor / 3)
                ))
            );
        }
